...
import de.keag.lager.panels.frame.bestellung.BestellungPosStatus;
 ...
 */
public class BestellungPosBeanDB extends BeanDB {
	...
	protected String getDeleteSQL() {
		if(deleteSQL==null){
			deleteSQL = "delete from bestellungsposition " +
						" where id = ? ";
			}
		return deleteSQL;
	}
	...
	public void deleteBean(Bean bean) throws SQLException, LagerException {
		BestellungPosBean baPosBean = (BestellungPosBean) bean;
	...
	protected String getUpdateSQL() {
		if(updateSQL==null){
		updateSQL = " update bestellungsposition " +
	...
	protected void updateBean(Bean bean) throws SQLException, LagerException {
		...
		try{
			...
			getUpdateStmt().setString(4, BestellungPosStatus.JavaToDB(baPosBean.getStatus()));
...
//		bean.setBeanDBStatus(BeanDBStatus.SELECT);
	...
	protected String getInsertSQL() {
		if(insertSQL==null){
			insertSQL = "insert into bestellungsposition " +
					"(id, fk_bestellung, fk_artikel, bestellmenge, liefertermin, status, " +
	...
	protected void insertBean(Bean bean) throws SQLException, LagerException {
		BestellungPosBean beanBaPos = (BestellungPosBean) bean;
		getInsertStmt().setInt(1, beanBaPos.getId());
		getInsertStmt().setInt(2, beanBaPos.getBestellungBean().getId());
		...
		getInsertStmt().setString(6, BestellungPosStatus.JavaToDB(beanBaPos.getStatus()));
		...
		try{
			...
			if (e.getSQLState().equals("23000")){
				if (e.getMessage().contains(Konstanten.SQL_ERROR_bestellungsposition_bestellung_artikel)){
...
//		beanBaPos.setBeanDBStatus(BeanDBStatus.SELECT);
	...
	public ArrayList<BestellungPosBean>  sucheAnhandBestellungBean(BestellungBean baBean, int depthLevel ) throws SQLException, LagerException{
		ArrayList<BestellungPosBean> resultList = new ArrayList<BestellungPosBean>();
		ResultSet rs;
		//sql bilden
		String sql = "select id, fk_bestellung, fk_artikel, " +
				"bestellmenge, liefertermin, status, fk_kostenstelle, fk_mengeneinheit, " +
				"fk_katalog, lieferantenbestellnummer, katalogseite, katalogpreis" +
				" from bestellungsposition where fk_bestellung = ? order by id";
		//SQL absetzen
...
//		baBean.loescheAlleBaPositionen();
		while(rs.next()){
			BestellungPosBean baPosBean = new BestellungPosBean();
			baPosBean.setId(rs.getInt("id"));
			baPosBean.setStatus(BestellungPosStatus.DbToJava(rs.getString("status")));
			if (baBean.getId()==rs.getInt("fk_bestellung")){
				baPosBean.setBestellungBean(baBean);
			}else{
			...
			baPosBean.setKostenstelle((KostenstelleBean)getKostenstelleBeanDB().selectAnhandID(rs.getInt("fk_kostenstelle"), 0, null));
			baPosBean.setMengenEinheitBean((MengenEinheitBean)getMengenEinheitBeanDB().selectAnhandID(rs.getInt("fk_mengeneinheit"), 0, null)); //Achtung! Nicht aus dem Artikelstamm.
			baPosBean.setKatalogBean((KatalogBean)getKatalogBeanDB().selectAnhandID(rs.getInt("fk_katalog"), 0, null));
