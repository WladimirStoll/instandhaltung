...
public class HalleListPaneView extends ListView{

	private JToolBar jToolBar = null;
	private JButton jButtonMenue = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private HalleListPaneController listPaneController;  //  @jve:decl-index=0:
	 ...
	 * 
	 */
	public HalleListPaneView(HalleListPaneController benutzerListPaneController,IModel iModel) {
		super();
		setiModel(iModel) ;
		setHalleListPaneController(benutzerListPaneController);
		initialize();
	}

	private void setHalleListPaneController(HalleListPaneController benutzerListPaneController) {
	...
	public void zeichneDich(IModel iModel) {
		...
		for(int i = 0; i<getJTable().getModel().getRowCount();i++){
			HalleBean rowModelHalleBean = (HalleBean)getJTable().getModel().getValueAt(i, Konstanten.COLUMN_COUNT_BIG);
			if (rowModelHalleBean == modelKnotenBean.getIBean()){
	...
	private JButton getJButtonNeu() {
		if (jButtonNeu == null) {
			...
			jButtonNeu.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					HalleListPaneView.this.erstelleNeuenSatz();
//					if (!prueferFehler()) return;
//					ModelKnotenBean modelKnotenBean = getiModel().getSelectedListModelKnotenBean();
//					if(modelKnotenBean.istGesamterInhaltGeaendert()){
//						zeigeFehlerDialogSatzGeaendert();
//					}else{
//						getHalleListPaneController().erstelleNeuenBenutzer();
//					}
				}
			});
	...
	private JButton getJButtonSpeichern() {
		if (jButtonSpeichern == null) {
			...
			jButtonSpeichern.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					HalleListPaneView.this.speichereSatz();
					//					if (!prueferFehler()) return;
//					HalleBean halleBean = (HalleBean)getiModel().
...
//					}else{
//						getHalleListPaneController().speichereBenutzer(halleBean);
//					}
//					
				}
			});
	...
	private JButton getJButtonLoeschen() {
		if (jButtonLoeschen == null) {
			...
			jButtonLoeschen.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					HalleListPaneView.this.loescheSatz();
//					HalleBean halleBean = (HalleBean)getiModel().getSelectedListModelKnotenBean().getIBean(); //aktuelle Bean holen
//					getHalleListPaneController().loescheBenutzer(halleBean);
	...
	private JButton getJButtonAbbrechen() {
		if (jButtonAbbrechen == null) {
			...
			jButtonAbbrechen.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
//					getHalleListPaneController().setzeNeuenAktivenController(); //klappt oder nicht = egal					
//					HalleBean halleBean = (HalleBean)getiModel().getSelectedListModelKnotenBean().getIBean(); //aktuelle Bean holen
//					getHalleListPaneController().abbrechenBenutzer(halleBean);
					HalleListPaneView.this.abbrechenSatz();
				}
			});
		}
	...
	protected ListSelectionListener getListSelectionListener() {
		if (listSelectionListener==null){
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				int aktuelleBeanPosition = HalleListPaneView.this.getiModel().getSelectedListModelKnotenBeanLaufendeNummer();
				...
				if (prueferFehler()){
					ModelKnotenBean modelKnotenBean = HalleListPaneView.this.getiModel().getSelectedListModelKnotenBean();
//					HalleBean halleBean = (HalleBean) modelKnotenBean.getIBean();
					ArrayList<Fehler> errors = HalleListPaneView.this.getIController().pruefeAktuellenSatz();
					...
					HalleListPaneView.this.getIController().aktualisiereAnzeige();
				}
			}
		...
		if (defaultTableModel == null) {
			defaultTableModel = new DefaultTableModel() {
				...
				public Object getValueAt(int rowIndex, int columnIndex) {
					...
					HalleModelKnotenBean halleModelKnotenBean = (HalleModelKnotenBean) HalleListPaneView.this.getiModel().getModelBeanList().get(rowIndex);
					// Aus dem Anforderungsknoten wird die
					// Anforderung(Nutzdaten) gelesen.
					HalleBean halleBean = halleModelKnotenBean.getIBean();
					...
					case 0:
						return "";
					case 1:
						return halleBean.getName();
					case Konstanten.COLUMN_COUNT_BIG: return halleBean;
					default:
...
//					return HalleModel.this.getModelBeanList().size();
					if (HalleListPaneView.this.getiModel().getModelBeanList()==null){
						return 0;
					}else {
						return HalleListPaneView.this.getiModel().getModelBeanList().size();
					}
				}
...
//	}
	...
	protected boolean prueferFehler() {
		...
		if (modelKnotenBean != null && modelKnotenBean.getIBean() != null) {
			HalleBean halleBean = (HalleBean) modelKnotenBean
					.getIBean();
...
//			});
	...
	public TableCellRenderer getStatusCellRenderer(int row, int column) {
		...
		if (column == 0){
			HalleModelKnotenBean benutzerModelKnotenBean = (HalleModelKnotenBean) HalleListPaneView.this.getiModel().getModelBeanList().get(row);
//			String iconError; 
