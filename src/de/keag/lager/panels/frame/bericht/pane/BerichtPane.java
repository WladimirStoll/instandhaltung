...
import de.keag.lager.core.JPanePanel;
import de.keag.lager.core.fehler.BenutzerOberflacheLagerException;
import de.keag.lager.core.main.Konstanten;
import de.keag.lager.core.main.Run;
import de.keag.lager.panels.frame.MainFrame;
import de.keag.lager.panels.frame.bericht.pane.liste.BerichtListView;
...
public class BerichtPane extends JPanePanel{
	...
	private BerichtPaneController paneController = null;
	private BerichtListView listPane;
	 ...
	 * 
	 */
	protected BerichtPane(BerichtPaneController paneController, IModel iModel) {
		super();
		initialize();
	}
	...
	private JPanel getJPanelTreeBasic() {
		if (jPanelTreeBasic == null) {
			...
			//Tree hinzufügen.
	        jPanelTreeBasic.add(BerichtPaneController.getOneInstance().getTreePane(), gridBagConstraints); //mit konstains
	...
	private JPanel getJPanelListBasic() {
		if (jPanelListBasic == null) {
			...
			//Liste hinzufügen.
			jPanelListBasic.add(BerichtPaneController.getOneInstance().getListPane(), gridBagConstraints);
	...
	private JPanel getJPanelSuchBasic() {
		if (jPanelSuchBasic == null) {
			...
			//SuchPane hinzufügen.
			jPanelSuchBasic.add(BerichtPaneController.getOneInstance().getSuchView(), gridBagConstraints); //mit konstains
	...
	public void setContentPane(MainFrame mainFrame) {
		mainFrame.setTitle(Konstanten.UEBERSCHRIFT_ANWENDUNG + Konstanten.SLASH + Konstanten.UEBERSCHRIFT_DRUCK);
	...
	private BerichtPaneController getPaneController() {
		return paneController;
	}

	private void setPaneController(BerichtPaneController paneController) {
		this.paneController = paneController;
	}

	protected BerichtListView getBaListPane() throws BenutzerOberflacheLagerException{
		if(listPane==null){
			BerichtPaneController.getOneInstance().getListPane();
//			throw new BenutzerOberflacheLagerException(new Fehler(12,FehlerTyp.FEHLER,"getBaListPane()"));
	...
	@Override
	public void setBlatt(JPanel blatt) {
		...
		getJPanelDetailsBasic().add(blatt,gridBagConstraints);
