...
public class EntnahmeTreeView extends TreeView implements ITreeBeobachter {
	 ...
	 * @param entnahmeTreePaneController 
	 * 
	 */
	protected EntnahmeTreeView(EntnahmeTreeController entnahmeTreePaneController) {
		super();
		setTreeController(entnahmeTreePaneController);
		initialize();
	}
	...
	public JTree getJTree() {
		if (jTree == null) {
			...
			GenericNode root;
			try {
				root = new GenericNode(new EntnahmePosModelKnotenBean(new EntnahmePosBean()));
			...
			// Aussehen von JTree
			jTree.setCellRenderer(new EntnahmePosTreeRenderer());
//			jTree = new JLagerTree();
...
//		if (root.getUserObject() instanceof EntnahmeModelKnotenBean){
//			//Anforderung wir im Tree gezeichnet.
//			TreePath angeklickterKnoten = null;
//			EntnahmeModelKnotenBean entnahmeModelKnotenBean = (EntnahmeModelKnotenBean)root.getUserObject();
//			for (ModelKnotenBean modelKnotenBeanKind : entnahmeModelKnotenBean.getKinderList()) {
//				if (modelKnotenBeanKind instanceof EntnahmePosModelKnotenBean){
...
//			if (modelKnotenBean==entnahmeModelKnotenBean){
...
//	}
	...
	private class EntnahmePosTreeRenderer extends DefaultTreeCellRenderer {
	    /**
		 * 
		 */
...
//					if (modelKnotenBean instanceof EntnahmeModelKnotenBean) {
//						EntnahmeModelKnotenBean entnahmeModelKnotenBean = (EntnahmeModelKnotenBean)modelKnotenBean;
//						String IconNameStatus = BeanDBStatus.JavaToIconName(entnahmeModelKnotenBean.getIBean().getBeanDBStatus());
//						Icon icon = Run.createCompoundIcon(Konstanten.ICON_ENTNAHME, IconNameStatus,iconError);
//						setIcon(icon);
//					}
//					;
					if (modelKnotenBean instanceof EntnahmePosModelKnotenBean) {
						EntnahmePosModelKnotenBean entnahmePosModelKnotenBean = (EntnahmePosModelKnotenBean)modelKnotenBean;
						String IconNameStatus = BeanDBStatus.JavaToIconName(entnahmePosModelKnotenBean.getIBean().getBeanDBStatus());
						Icon icon = Run.createCompoundIcon(Konstanten.ICON_ENTNAHMENSPOSITION, IconNameStatus,iconError);
						setIcon(icon);
					}
					;
