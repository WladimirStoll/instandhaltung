...
import de.keag.lager.panels.frame.entnahme.pane.liste.EntnahmeListView;
...
import de.keag.lager.panels.frame.entnahme.pane.tree.EntnahmeTreeView;
...
public class EntnahmePane extends JPanel implements IPanel  {
	...
	private EntnahmePaneController entnahmePaneController = null;
	private EntnahmeListView entnahmeListPane;
	 ...
	 * 
	 */
	protected EntnahmePane(EntnahmePaneController entnahmePaneController, IModel iModel) {
		super();
		initialize();
	}
	...
	private JPanel getJPanelTreeBasic() {
		if (jPanelTreeBasic == null) {
			...
			//Tree hinzufügen.
	        jPanelTreeBasic.add(EntnahmePaneController.getOneInstance().getTreePane(), gridBagConstraints); //mit konstains
	...
	private JPanel getJPanelListBasic() {
		if (jPanelListBasic == null) {
			...
			//Liste hinzufügen.
			jPanelListBasic.add(EntnahmePaneController.getOneInstance().getListPane(), gridBagConstraints);
	...
	private JPanel getJPanelSuchBasic() {
		if (jPanelSuchBasic == null) {
			...
			//SuchPane hinzufügen.
			jPanelSuchBasic.add(EntnahmePaneController.getOneInstance().getSuchView(), gridBagConstraints); //mit konstains
	...
	public void setContentPane(MainFrame mainFrame) {
		mainFrame.setTitle(Konstanten.UEBERSCHRIFT_ANWENDUNG + Konstanten.SLASH + Konstanten.UEBERSCHRIFT_ENTNAHME);
	...
	private EntnahmePaneController getEntnahmePaneController() {
		return entnahmePaneController;
	}

	private void setEntnahmePaneController(EntnahmePaneController entnahmePaneController) {
		this.entnahmePaneController = entnahmePaneController;
	}

	protected EntnahmeListView getEntnahmeListPane() throws BenutzerOberflacheLagerException{
		if(entnahmeListPane==null){
			EntnahmePaneController.getOneInstance().getListPane();
//			throw new BenutzerOberflacheLagerException(new Fehler(12,FehlerTyp.FEHLER,"getEntnahmeListPane()"));
		}
		return entnahmeListPane;
	}

	public void setBlatt(JPanel entnahmeBlatt) {
		...
		getJPanelDetailsBasic().add(entnahmeBlatt,gridBagConstraints);
	...
	private void setiEntnahmeModel(IModel iEntnahmeModel) {
		this.iModel = iEntnahmeModel;
	}
