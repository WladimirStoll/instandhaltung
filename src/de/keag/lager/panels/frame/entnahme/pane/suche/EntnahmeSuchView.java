...
public class EntnahmeSuchView extends SuchView {
	 ...
	 * 
	 */
	protected EntnahmeSuchView(EntnahmeSuchController entnahmeSuchPaneController, IModel iModel) {
		super();
		setSuchController(entnahmeSuchPaneController);
		setiModel(iModel);
		initialize();
	}
		...
		if (jButtonSuchen == null) {
			...
			jButtonSuchen.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					EntnahmePosSuchBean entnahmePosSuchBean = new EntnahmePosSuchBean();
					entnahmePosSuchBean.setErstellungsDatumVon(getJPanelDatumVon().getDate());
					entnahmePosSuchBean.setErstellungsDatumBis(getJPanelDatumBis().getDate());
					entnahmePosSuchBean.setEntnahmeStatus((EntnahmeStatus)getJComboBoxStatus().getSelectedItem());
					getSuchController().suchen(entnahmePosSuchBean);
				}
			});
		}
		...
		if (jPanelUnten == null) {
			...
			jPanelUnten.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					EntnahmeSuchView.super.mouseClickedFehler(e, (JComboBox)e.getSource());
				}
			});
		}
		return jPanelUnten;
	...
	private JComboBox getJComboBoxFehler() {
		if (jComboBoxFehler == null) {
			...
			jComboBoxFehler.addMouseListener(new java.awt.event.MouseAdapter() {
				@Override
				public void mouseClicked(java.awt.event.MouseEvent e) {
					EntnahmeSuchView.this.mouseClickedFehler(e, getJComboBoxFehler());
				}
			});
		}
	...
	private JDateChooser getJPanelDatumVon() {
		if (jPanelDatumVon == null) {
			...
			int counter = 0;
			while (jPanelDatumVon==null&&counter++<10){
				try{
					...
					Log.log().severe("counter:"+new Integer(counter).toString());
					e.printStackTrace();
					try {
						Thread.sleep(200);
	...
	private JComboBox getJComboBoxStatus() {
		if (jComboBoxStatus == null) {
			EntnahmeStatus[] petEntnahmeen = {
					EntnahmeStatus.OFFEN,
					EntnahmeStatus.BESTAETIGT,
					EntnahmeStatus.GELOESCHT,
					EntnahmeStatus.FEHLERHAFT,
					EntnahmeStatus.ALLE
			};			
			jComboBoxStatus = new JComboBox(petEntnahmeen);
...
//					if (!prueferFehler()) return;
	...
	private JDateChooser getJPanelDatumBis() {
		if (jPanelDatumBis == null) {
			int counter = 0;
			while (jPanelDatumBis==null&&counter++<10){
				try{
