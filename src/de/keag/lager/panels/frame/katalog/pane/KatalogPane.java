...
import de.keag.lager.panels.frame.katalog.pane.liste.KatalogListView;
...
public class KatalogPane extends JPanel implements IPanel  {
	...
	private KatalogPaneController einlagerungPaneController = null;
	private KatalogListView einlagerungListPane;
	 ...
	 * 
	 */
	protected KatalogPane(KatalogPaneController einlagerungPaneController, IModel iModel) {
		super();
		initialize();
	}
	...
	private JPanel getJPanelTreeBasic() {
		if (jPanelTreeBasic == null) {
			...
			//Tree hinzufügen.
	        jPanelTreeBasic.add(KatalogPaneController.getOneInstance().getTreePane(), gridBagConstraints); //mit konstains
	...
	private JPanel getJPanelListBasic() {
		if (jPanelListBasic == null) {
			...
			//Liste hinzufügen.
			jPanelListBasic.add(KatalogPaneController.getOneInstance().getListPane(), gridBagConstraints);
	...
	private JPanel getJPanelSuchBasic() {
		if (jPanelSuchBasic == null) {
			...
			//SuchPane hinzufügen.
			jPanelSuchBasic.add(KatalogPaneController.getOneInstance().getSuchView(), gridBagConstraints); //mit konstains
	...
	public void setContentPane(MainFrame mainFrame) {
		mainFrame.setTitle(Konstanten.UEBERSCHRIFT_ANWENDUNG + Konstanten.SLASH + Konstanten.UEBERSCHRIFT_KATALOG);
	...
	private KatalogPaneController getKatalogPaneController() {
		return einlagerungPaneController;
	}

	private void setKatalogPaneController(KatalogPaneController einlagerungPaneController) {
		this.einlagerungPaneController = einlagerungPaneController;
	}

	protected KatalogListView getKatalogListPane() throws BenutzerOberflacheLagerException{
		if(einlagerungListPane==null){
			KatalogPaneController.getOneInstance().getListPane();
//			throw new BenutzerOberflacheLagerException(new Fehler(12,FehlerTyp.FEHLER,"getKatalogListPane()"));
		}
	...
	private void setiKatalogModel(IModel iKatalogModel) {
		this.iModel = iKatalogModel;
	}
