...
public class EtageMCView extends JDialog implements IEtageMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private EtageMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IEtageMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected EtageMCView(Frame frame, EtageMCController mCController, IEtageMCModel iMCModel) {
		super(frame);
		this.setModal(true);
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						EtageBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						EtageMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private EtageMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			EtageMCController lieferantMCController) {
	...
	private AbstractTableModel getAbstractTableModel() {
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				private String[] columnNames = {"Etage","in der Halle"}; 
				...
				public Object getValueAt(int zeile, int spalte) {
					try{
						...
						case 1: return getiMCModel().getBeans().get(zeile).getHalleBean().getName();  
	...
	private IEtageMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IEtageMCModel iMCModel) {
