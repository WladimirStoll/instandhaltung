...
public class LieferantenBestellnummerMCView extends JDialog implements ILieferantenBestellnummerMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private LieferantenBestellnummerMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private ILieferantenBestellnummerMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected LieferantenBestellnummerMCView(Frame frame, LieferantenBestellnummerMCController mCController, ILieferantenBestellnummerMCModel iMCModel) {
		super(frame);
		this.setModal(true);
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						LieferantenBestellnummerBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						LieferantenBestellnummerMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private LieferantenBestellnummerMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			LieferantenBestellnummerMCController lieferantMCController) {
	...
	private AbstractTableModel getAbstractTableModel() {
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				private String[] columnNames = {"Bestellnummer","Katalog","Seite","Artikel KEG","Artikel Typ","Preis"}; 
				...
				public Object getValueAt(int row, int col) {
					try{
						switch (col){
						case 0: return getiMCModel().getBeans().get(row).getBestellnummer();  
						case 1: return getiMCModel().getBeans().get(row).getKatalogBean().getName();  
						case 2: return getiMCModel().getBeans().get(row).getKatalogseite();  
						case 3: return getiMCModel().getBeans().get(row).getArtikel().getKeg_nr().toString(); 
						case 4: return getiMCModel().getBeans().get(row).getArtikel().getTyp();  
						case 5: return getiMCModel().getBeans().get(row).getPreis().toString();  
	...
	private ILieferantenBestellnummerMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(ILieferantenBestellnummerMCModel iMCModel) {
