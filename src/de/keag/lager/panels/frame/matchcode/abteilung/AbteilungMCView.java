...
public class AbteilungMCView extends JDialog implements IAbteilungMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private AbteilungMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IAbteilungMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected AbteilungMCView(Frame frame, AbteilungMCController mCController, IAbteilungMCModel iMCModel) {
		super(frame);
		this.setModal(true);
	...
	private void initialize() {
		...
		this.setTitle("Auswahl Abteilung");
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						AbteilungBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						AbteilungMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private AbteilungMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			AbteilungMCController lieferantMCController) {
		...
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				public Object getValueAt(int zeile, int spalte) {
					try{
						switch (spalte){
						case 0: return getiMCModel().getBeans().get(zeile).getAbteilungName();  
	...
	private IAbteilungMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IAbteilungMCModel iMCModel) {
