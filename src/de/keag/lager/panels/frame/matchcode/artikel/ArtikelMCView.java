...
public class ArtikelMCView extends JDialog implements IArtikelMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private ArtikelMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IArtikelMCModel iMCModel;  //  @jve:decl-index=0:
	...
	private LhBean lhBean;  //  @jve:decl-index=0:
	
	
	/**
	 * @param frame
	 */
	protected ArtikelMCView(Frame frame, ArtikelMCController mCController, IArtikelMCModel iMCModel) {
		super(frame);
		this.setModal(true);
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						ArtikelBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						ArtikelMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private ArtikelMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			ArtikelMCController lieferantMCController) {
	...
	private AbstractTableModel getAbstractTableModel() {
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				private String[] columnNames = {"KEG","Bezeichnung","Hersteller","Typ"}; 
				...
				public Object getValueAt(int row, int col) {
					try{
						switch (col){
						case 0: return getiMCModel().getBeans().get(row).getKeg_nr();  
						case 1: return getiMCModel().getBeans().get(row).getBezeichnung();  
						case 2: return getiMCModel().getBeans().get(row).getHersteller().getName();  
						case 3: return getiMCModel().getBeans().get(row).getTyp();  
	...
	private IArtikelMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IArtikelMCModel iMCModel) {
...
//		this.revalidate(); //Layout-Manager tut seinen JOB, und richtet				this.invalidate();
	...
	private JPanel getJPanelSuche() {
		if (jPanelSuche == null) {
			...
			jPanelSuche.add(getJButtonMatchCodeLieferant(), gridBagConstraints11);
		}
		return jPanelSuche;
	}
			...
			jButtonAuswahlEinschraenken
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							...
							//Hersteller
							getiMCModel().getSuchKriterien().setHersteller(getLhBean());
							//KEG
							Integer keg_nr;
							try{
	...
	private JTextField getJTextFieldHersteller() {
		if (jTextFieldHersteller == null) {
			...
			jTextFieldHersteller.setEditable(false);
		}
		...
		if (jButtonMatchCodeLieferant == null) {
			...
			jButtonMatchCodeLieferant
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							try {
									getMCController().holeLieferanten();
	...
	public void setMatchCodeLieferanten(LhBean lhBean) {
		setLhBean(lhBean);
		getJTextFieldHersteller().setText(getLhBean().getName());
	}

	private LhBean getLhBean() {
		if (lhBean==null){
			lhBean = new LhBean();
		}
		return lhBean;
	...
	private void setLhBean(LhBean lhBean) {
		this.lhBean = lhBean;
	}
