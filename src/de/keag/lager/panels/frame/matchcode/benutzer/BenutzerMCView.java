...
public class BenutzerMCView extends JDialog implements IBenutzerMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private BenutzerMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IBenutzerMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected BenutzerMCView(Frame frame, BenutzerMCController mCController, IBenutzerMCModel iMCModel) {
		super(frame);
		this.setModal(true);
	...
	private void initialize() {
		...
		this.setTitle("Auswahl Benutzer");
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						BenutzerBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						BenutzerMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private BenutzerMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			BenutzerMCController lieferantMCController) {
			...
			abstractTableModel = new AbstractTableModel() {
				...
				public Object getValueAt(int zeile, int spalte) {
					try{
						...
						case 1: return getiMCModel().getBeans().get(zeile).getVorname();
	...
	private IBenutzerMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IBenutzerMCModel iMCModel) {
