...
public class WartungsArtMCView extends JDialog implements IWartungsArtMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private WartungsArtMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IWartungsArtMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected WartungsArtMCView(Frame frame, WartungsArtMCController mCController, IWartungsArtMCModel iMCModel) {
		super(frame);
		this.setModal(true);
	...
	private void initialize() {
		...
		this.setTitle("Auswahl  Wartungsart");
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						WartungsArtBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						WartungsArtMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private WartungsArtMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			WartungsArtMCController lieferantMCController) {
	...
	private AbstractTableModel getAbstractTableModel() {
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				private String[] columnNames = {"Wartungsart","Wiederkehrend"}; 
				...
				public Object getValueAt(int zeile, int spalte) {
					try{
						switch (spalte){
						case 0: return getiMCModel().getBeans().get(zeile).getName();  
						case 1: return WartungsArtIntervallFaehigEnum.JavaToView(getiMCModel().getBeans().get(zeile).getIntervallFaehig());  
	...
	private IWartungsArtMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IWartungsArtMCModel iMCModel) {
