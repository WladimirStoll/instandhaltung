...
public class ZugriffsrechtMCView extends JDialog implements IZugriffsrechtMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private ZugriffsrechtMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IZugriffsrechtMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected ZugriffsrechtMCView(Frame frame, ZugriffsrechtMCController mCController, IZugriffsrechtMCModel iMCModel) {
		super(frame);
		this.setModal(true);
	...
	private void initialize() {
		...
		this.setTitle("Auswahl Zugriffsrecht");
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						ZugriffsrechtBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						ZugriffsrechtMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private ZugriffsrechtMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			ZugriffsrechtMCController lieferantMCController) {
		...
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				public Object getValueAt(int zeile, int spalte) {
					try{
						switch (spalte){
						case 0: return getiMCModel().getBeans().get(zeile).getZugriffsrechtName();  
	...
	private IZugriffsrechtMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IZugriffsrechtMCModel iMCModel) {
