...
public class MengeneinheitMCView extends JDialog implements IMengeneinheitMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private MengeneinheitMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IMengeneinheitMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected MengeneinheitMCView(Frame frame, MengeneinheitMCController mCController, IMengeneinheitMCModel iMCModel) {
		super(frame);
		this.setModal(true);
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						MengenEinheitBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						MengeneinheitMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private MengeneinheitMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			MengeneinheitMCController lieferantMCController) {
	...
	private AbstractTableModel getAbstractTableModel() {
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				private String[] columnNames = {"Mengeneinheit"}; 
	...
	private IMengeneinheitMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IMengeneinheitMCModel iMCModel) {
