...
public class WartungsTypMCView extends JDialog implements IWartungsTypMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private WartungsTypMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IWartungsTypMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected WartungsTypMCView(Frame frame, WartungsTypMCController mCController, IWartungsTypMCModel iMCModel) {
		super(frame);
		this.setModal(true);
	...
	private void initialize() {
		...
		this.setTitle("Auswahl  Wartungsyp");
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						WartungsTypBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						WartungsTypMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private WartungsTypMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			WartungsTypMCController lieferantMCController) {
	...
	private AbstractTableModel getAbstractTableModel() {
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				private String[] columnNames = {"Wartungstyp"}; 
	...
	private IWartungsTypMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IWartungsTypMCModel iMCModel) {
