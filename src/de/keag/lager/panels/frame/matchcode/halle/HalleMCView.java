...
public class HalleMCView extends JDialog implements IHalleMCBeobachter {

	private static final long serialVersionUID = 1L;
	private JPanel jContentPane = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private HalleMCController mCController;  //  @jve:decl-index=0:
	private AbstractTableModel abstractTableModel;
	private IHalleMCModel iMCModel;  //  @jve:decl-index=0:
	 ...
	 * @param frame
	 */
	protected HalleMCView(Frame frame, HalleMCController mCController, IHalleMCModel iMCModel) {
		super(frame);
		this.setModal(true);
...
//				getMCController().fensterIstGeschlossen();						
			...
			jTable.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(e.getClickCount()==2){
						...
						HalleBean bean = getiMCModel().getBeans().get(jTable.getSelectedRow());
						getMCController().setGewaehlteBean(bean);
						HalleMCView.this.setVisible(false);
						getMCController().fensterIstGeschlossen();
					};
				}
			});
	...
	private HalleMCController getMCController() {
		return mCController;
	}

	private void setMCController(
			HalleMCController lieferantMCController) {
	...
	private AbstractTableModel getAbstractTableModel() {
		if(abstractTableModel==null){
			abstractTableModel = new AbstractTableModel() {
				...
				private String[] columnNames = {"Halle"}; 
	...
	private IHalleMCModel getiMCModel() {
		return iMCModel;
	}

	private void setiMCModel(IHalleMCModel iMCModel) {
