...
import de.keag.lager.panels.frame.wartung.pane.liste.WartungListPaneView;
...
public class WartungPane extends JPanel implements IPanel  {
	...
	private WartungPaneController benutzerPaneController = null;  //  @jve:decl-index=0:
	private WartungListPaneView benutzerListPane;
	private IModel iWartungModel  = null;  //  @jve:decl-index=0:
	/**
	 * This method initializes 
	 * @param benutzerPaneController 
	 * 
	 */
	protected WartungPane(WartungPaneController benutzerPaneController, IModel iWartungModel) {
		super();
		initialize();
	}
...
//				);
	...
	private JPanel getJPanelTreeBasic() {
		if (jPanelTreeBasic == null) {
			...
			//Tree hinzufügen.
	        jPanelTreeBasic.add(WartungPaneController.getOneInstance().getTreePane(), gridBagConstraints); //mit konstains
	...
	private JPanel getJPanelListBasic() {
		if (jPanelListBasic == null) {
			...
			//Liste hinzufügen.
			jPanelListBasic.add(WartungPaneController.getOneInstance().getListPane(), gridBagConstraints);
	...
	private JPanel getJPanelSuchBasic() {
		if (jPanelSuchBasic == null) {
			...
			//SuchPane hinzufügen.
			jPanelSuchBasic.add(WartungPaneController.getOneInstance().getSuchView(), gridBagConstraints); //mit konstains
	...
	public void setContentPane(MainFrame mainFrame) {
		mainFrame.setTitle(Konstanten.UEBERSCHRIFT_ANWENDUNG + Konstanten.SLASH + Konstanten.UEBERSCHRIFT_WARTUNG);
...
//		Zugriffsrechtpruefung.getOneInstance().jComponentCheck(this);
	...
	private WartungPaneController getWartungPaneController() {
		return benutzerPaneController;
	}

	private void setBaPaneController(WartungPaneController benutzerPaneController) {
		this.benutzerPaneController = benutzerPaneController;
	}

	protected WartungListPaneView getWartungListPane() throws BenutzerOberflacheLagerException{
		if(benutzerListPane==null){
			WartungPaneController.getOneInstance().getListPane();
//			throw new BenutzerOberflacheLagerException(new Fehler(12,FehlerTyp.FEHLER,"getBaListPane()"));
	...
	private IModel getiWartungModel() {
		return iWartungModel;
	}

	private void setiWartungModel(IModel iWartungModel) {
		this.iWartungModel = iWartungModel;
	}
