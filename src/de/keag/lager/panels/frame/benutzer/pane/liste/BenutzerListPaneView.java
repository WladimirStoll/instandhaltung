...
public class BenutzerListPaneView extends ListView{

	private JToolBar jToolBar = null;
	private JButton jButtonMenue = null;
	private JScrollPane jScrollPane = null;
	private JTable jTable = null;
	private BenutzerListPaneController benutzerListPaneController;  //  @jve:decl-index=0:
	 ...
	 * 
	 */
	public BenutzerListPaneView(BenutzerListPaneController benutzerListPaneController,IModel iModel) {
		super();
		setiModel(iModel) ;
		setBenutzerListPaneController(benutzerListPaneController);
		initialize();
	}

	private void setBenutzerListPaneController(BenutzerListPaneController benutzerListPaneController) {
	...
	public void zeichneDich(IModel iModel) {
		...
		for(int i = 0; i<getJTable().getModel().getRowCount();i++){
			BenutzerBean rowModelBenutzerBean = (BenutzerBean)getJTable().getModel().getValueAt(i, Konstanten.COLUMN_COUNT_BIG);
			if (rowModelBenutzerBean == modelKnotenBean.getIBean()){
	...
	private JButton getJButtonNeu() {
		if (jButtonNeu == null) {
			...
			jButtonNeu.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					BenutzerListPaneView.this.erstelleNeuenSatz();
//					if (!prueferFehler()) return;
//					ModelKnotenBean modelKnotenBean = getiModel().getSelectedListModelKnotenBean();
//					if(modelKnotenBean.istGesamterInhaltGeaendert()){
//						zeigeFehlerDialogSatzGeaendert();
//					}else{
//						getBenutzerListPaneController().erstelleNeuenBenutzer();
//					}
				}
			});
	...
	private JButton getJButtonSpeichern() {
		if (jButtonSpeichern == null) {
			...
			jButtonSpeichern.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					BenutzerListPaneView.this.speichereSatz();
					//					if (!prueferFehler()) return;
//					BenutzerBean benutzerBean = (BenutzerBean)getiModel().
...
//					}else{
//						getBenutzerListPaneController().speichereBenutzer(benutzerBean);
//					}
//					
				}
			});
	...
	private JButton getJButtonLoeschen() {
		if (jButtonLoeschen == null) {
			...
			jButtonLoeschen.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					BenutzerListPaneView.this.loescheSatz();
//					BenutzerBean benutzerBean = (BenutzerBean)getiModel().getSelectedListModelKnotenBean().getIBean(); //aktuelle Bean holen
//					getBenutzerListPaneController().loescheBenutzer(benutzerBean);
	...
	private JButton getJButtonAbbrechen() {
		if (jButtonAbbrechen == null) {
			...
			jButtonAbbrechen.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
//					getBenutzerListPaneController().setzeNeuenAktivenController(); //klappt oder nicht = egal					
//					BenutzerBean benutzerBean = (BenutzerBean)getiModel().getSelectedListModelKnotenBean().getIBean(); //aktuelle Bean holen
//					getBenutzerListPaneController().abbrechenBenutzer(benutzerBean);
					BenutzerListPaneView.this.abbrechenSatz();
				}
			});
		}
	...
	protected ListSelectionListener getListSelectionListener() {
		if (listSelectionListener==null){
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				int aktuelleBeanPosition = BenutzerListPaneView.this.getiModel().getSelectedListModelKnotenBeanLaufendeNummer();
				...
				if (prueferFehler()){
					ModelKnotenBean modelKnotenBean = BenutzerListPaneView.this.getiModel().getSelectedListModelKnotenBean();
//					BenutzerBean benutzerBean = (BenutzerBean) modelKnotenBean.getIBean();
					ArrayList<Fehler> errors = BenutzerListPaneView.this.getIController().pruefeAktuellenSatz();
					...
					BenutzerListPaneView.this.getIController().aktualisiereAnzeige();
				}
			}
	...
	private DefaultTableModel getDefaultTableModel() {
		if (defaultTableModel == null) {
			defaultTableModel = new DefaultTableModel() {
				private String[] benutzerColumnNames = {"ID","Vorname",
				...
				public Object getValueAt(int rowIndex, int columnIndex) {
					...
					BenutzerModelKnotenBean benutzerModelKnotenBean = (BenutzerModelKnotenBean) BenutzerListPaneView.this.getiModel().getModelBeanList().get(rowIndex);
					// Aus dem Anforderungsknoten wird die
					// Anforderung(Nutzdaten) gelesen.
					BenutzerBean BenutzerBean = benutzerModelKnotenBean.getIBean();
					// je Spalte wird ein entsprechendes Feld(Inalt der Spalte)
					// genommen.
					switch (columnIndex) {
					case 0:
						return BenutzerBean.getId();
					case 1:
						return BenutzerBean.getVorname();
					case 2:
						return BenutzerBean.getName();
					case 3:
						return BenutzerBean.getLoginName();
					case 4:
						return BenutzerBean.getEmail();
					case 5:
						return BenutzerBean.getCopi_ba_email();
					case Konstanten.COLUMN_COUNT_BIG: return BenutzerBean;
					default:
...
//					return BenutzerModel.this.getModelBeanList().size();
					if (BenutzerListPaneView.this.getiModel().getModelBeanList()==null){
						return 0;
					}else {
						return BenutzerListPaneView.this.getiModel().getModelBeanList().size();
					}
				}
	...
	protected boolean prueferFehler() {
		...
		if (modelKnotenBean != null && modelKnotenBean.getIBean() != null) {
			BenutzerBean benutzerBean = (BenutzerBean) modelKnotenBean
					.getIBean();
...
//			});
	...
	public TableCellRenderer getStatusCellRenderer(int row, int column) {
		...
		if (column == 0){
			BenutzerModelKnotenBean benutzerModelKnotenBean = (BenutzerModelKnotenBean) BenutzerListPaneView.this.getiModel().getModelBeanList().get(row);
//			String iconError; 
