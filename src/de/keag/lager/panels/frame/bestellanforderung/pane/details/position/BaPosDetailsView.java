...
public class BaPosDetailsView extends DetailsView implements IDetailsBeobachter, PropertyChangeListener  {
...
//	private ModelKnotenBean baPosModelBean = null;  //  @jve:decl-index=0:
	 ...
	 */
	public BaPosDetailsView(BaPosDetailsController baPosDetailsController) {
		super();
	...
	public void zeichneDich(ModelKnotenBean baPosModelBean, IModel iModel) {
		...
		if(baPosModelBean!=null){
			if (baPosModelBean.getSammelKnotenTypENUM() == ModelKnotenTyp.ANFORDERUNGSPOSITION){
				setModelBean(baPosModelBean);//merken
				BaPosBean baPosBean = (BaPosBean) baPosModelBean.getIBean();
				//id anzeigen
				...
				//Bestellmenge 
				getJTextBestellmenge().removePropertyChangeListener("value", this);
				getJTextBestellmenge().setValue(baPosBean.getBestellmenge());
				getJTextBestellmenge().addPropertyChangeListener("value", this);
				...
				//Liefertermin
				getJTextLiefertermin().removePropertyChangeListener("value", this);
				getJTextLiefertermin().setValue(baPosBean.getLiefertermin());
				getJTextLiefertermin().addPropertyChangeListener("value", this);
				...
				BaBean baBean = (BaBean)baPosModelBean.getVater().getIBean();
...
//			jTextBestellmenge = new JFormattedTextField(new EmptyNumberFormatter());
			...
			jTextBestellmenge.addKeyListener(this);
...
//		jTextLiefertermin = new JFormattedTextField(new EmptyNumberFormatter());
		if (jTextLiefertermin == null) {
			...
			jTextLiefertermin.addKeyListener(this);
			...
			jButtonMatchcodeKostenstelle
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							try {
								if (pruefeFehler()){
									((BaPosDetailsController)getController()).holeKostenstelle();
								}
		...
		if (jButtonMatchcodeArtikel == null) {
			...
			jButtonMatchcodeArtikel.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						if (pruefeFehler()){
							((BaPosDetailsController)getController()).holeArtikel();
						}
...
//	}
		...
		if (jComboBoxFehler == null) {
			...
			jComboBoxFehler.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					BaPosDetailsView.super.mouseClickedFehler(e, (JComboBox)e.getSource());
				}
			});
		}
	...
	public void setKostenstelle(KostenstelleBean kostenstelleBean) {
		...
		((BaPosBean)getModelBean().getIBean()).setKostenstelle(kostenstelleBean);
		...
		if (jButtonMatchcodeLieferantenBestellnummer == null) {
			...
			jButtonMatchcodeLieferantenBestellnummer.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (pruefeFehler()){
						try {
							((BaPosDetailsController)getController()).sucheLieferantenBestellnummerByArtikelId(artikel_id);
						} catch (LagerException e1) {
							getJComboBoxFehler().addItem(new Fehler(24,FehlerTyp.FEHLER,e1.getMessage()));
							e1.printStackTrace();
						}
			...
			jButtonMatchcodeMengeneinheit
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							try {
								if (pruefeFehler()){
									((BaPosDetailsController)getController()).holeMengeneinheit();
								}
							} catch (SQLException e1) {
								getJComboBoxFehler().addItem(new Fehler(24,FehlerTyp.FEHLER,e1.getMessage()));
								e1.printStackTrace();
	...
	public void setMengeneinheit(MengenEinheitBean mengenEinheitBean) {
		//Das Model wird geändert.
		((BaPosBean)getModelBean().getIBean()).setMengenEinheitBean(mengenEinheitBean);
		//Alle Beobachter des Models werden über eine Änderung benachrichtigt.
		getController().ausgewaehlterKnotenIstGeandert();
	}
	...
	private void leseAusgetJTextLiefertermin() {
		BaPosBean baPosBean = (BaPosBean)getModelBean().getIBean();
		try {
			Object o = getJTextLiefertermin().getValue();
			if(o!=null&&(o instanceof Long)){
				long kw = ((Long)o).longValue();
				baPosBean.setLiefertermin(new Integer((int)kw));
			}else if(o!=null&&(o instanceof Integer)){
				int kw = ((Integer)o).intValue();
				baPosBean.setLiefertermin(new Integer(kw));
			}
		} catch (NumberFormatException ex) {
		}
	}
