...
public class BaPane extends JPanel implements IPanel  {
	...
	private BaPaneController baPaneController = null;
	private BaListView baListPane;
	 ...
	 * 
	 */
	protected BaPane(BaPaneController baPaneController, IModel iModel) {
		super();
		initialize();
	}
	...
	private JPanel getJPanelTreeBasic() {
		if (jPanelTreeBasic == null) {
			...
			//Tree hinzufügen.
	        jPanelTreeBasic.add(BaPaneController.getOneInstance().getTreePane(), gridBagConstraints); //mit konstains
	...
	private JPanel getJPanelListBasic() {
		if (jPanelListBasic == null) {
			...
			//Liste hinzufügen.
			jPanelListBasic.add(BaPaneController.getOneInstance().getListPane(), gridBagConstraints);
	...
	private JPanel getJPanelSuchBasic() {
		if (jPanelSuchBasic == null) {
			...
			//SuchPane hinzufügen.
			jPanelSuchBasic.add(BaPaneController.getOneInstance().getSuchView(), gridBagConstraints); //mit konstains
	...
	public void setContentPane(MainFrame mainFrame) {
		mainFrame.setTitle(Konstanten.UEBERSCHRIFT_ANWENDUNG + Konstanten.SLASH + Konstanten.UEBERSCHRIFT_BESTELLANFORDERUNGSLISTE);
	...
	private BaPaneController getBaPaneController() {
		return baPaneController;
	}

	private void setBaPaneController(BaPaneController baPaneController) {
		this.baPaneController = baPaneController;
	}

	protected BaListView getBaListPane() throws BenutzerOberflacheLagerException{
		if(baListPane==null){
			BaPaneController.getOneInstance().getListPane();
