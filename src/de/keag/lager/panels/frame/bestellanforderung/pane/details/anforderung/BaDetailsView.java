...
import de.keag.lager.panels.frame.matchcode.artikel.ArtikelMCView;
...
public class BaDetailsView extends DetailsView  {
...
//	private ListSelectionListener listSelectionListener;
	 ...
	 */
	public BaDetailsView(BaDetailsController baBlattController) {
		super();
	...
	private void initialize() {
		...
		this.addFocusListener(new java.awt.event.FocusAdapter() {   
			public void focusGained(java.awt.event.FocusEvent e) {
				BaDetailsView.this.setBorder(BorderFactory.createLineBorder(SystemColor.activeCaption, 5));
				Log.log().finest("focusGained()"); // TODO Auto-generated Event stub focusGained()
			}
			public void focusLost(java.awt.event.FocusEvent e) {
				BaDetailsView.this.setBorder(BorderFactory.createLineBorder(SystemColor.inactiveCaption, 5));
					BaDetailsView.this.uebernehmeDaten();
	...
	protected void uebernehmeDaten() {
		if (getModelBean()!=null){
			if (getModelBean().getIBean()!=null){
				if (getModelBean().getIBean() instanceof BaBean){
					BaBean baBean = (BaBean)getModelBean().getIBean();
							...
							try {
     							if (baBean.getLhBean()!=null){
		    						...
		    						if(lhName.compareToIgnoreCase(baBean.getLhBean().getName())!=0){
				    					...
				    					LhBean lhBean = ((BaDetailsController)getController()).getLhAnhandName(lhName); //Hole Lieferanten von DB
								    ...
								    LhBean lhBean = ((BaDetailsController)getController()).getLhAnhandName(lhName);
									baBean.setLhBean(lhBean); //Der aktuelle Lieferant wird durch einen neuen ausgetauscht.
							    }
			...
			jButtonMatchCodeLieferant
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							try {
								if (pruefeFehler()){
									((BaDetailsController)getController()).holeLieferanten();
								}
	...
	public void zeichneDich(ModelKnotenBean baModelBean, IModel iModel) {
		if(baModelBean!=null){
			if (baModelBean.getSammelKnotenTypENUM() == ModelKnotenTyp.ANFORDERUNG){
				getJComboBoxFehler().removeAllItems();
				setModelBean(baModelBean);//merken
				BaBean baBean = (BaBean) baModelBean.getIBean();
				getJTextFieldId().setText(baBean.getId().toString());
				getJTextFieldId().setEnabled(false);
				getJTextFieldStatus().setText(AnforderungStatus.JavaToView(baBean.getStatus()));
				if (baBean.getLhBean()!=null){
	...
	private JComboBox getJComboBoxFehler() {
		if (jComboBoxFehler == null) {
			...
			jComboBoxFehler.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					BaDetailsView.super.mouseClickedFehler(e, (JComboBox)e.getSource());
				}
			});
		}
	...
	private JButton getJButtonNeu() {
		if (jButtonNeu == null) {
			...
			jButtonNeu.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (pruefeFehler()){
						getController().erstelleNeuenSatz(ModelKnotenTyp.ANFORDERUNGSPOSITION);
					}
				}
			});
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (pruefeFehler()){
						if (getJTable().getSelectedRow()>=0){
							BaPosBean baPosBean = (BaPosBean) getJTable().getModel().getValueAt(getJTable().getSelectedRow(), 999);
		...
		if (jTextFieldEmail == null) {
			...
			jTextFieldEmail.addKeyListener(new java.awt.event.KeyAdapter() {
				public void keyReleased(KeyEvent keyEvent) {
					String email = BaDetailsView.this.getJTextFieldEmail().getText();
					setMatchCodeEmail(null,email);
//					System.out.println("keyReased:"+BaDetailsView.this.getJTextFieldEmail().getText());
				}
				@Override
				public void keyTyped(KeyEvent e) {
//					System.out.println("keyTyped:"+BaDetailsView.this.getJTextFieldEmail().getText());
				}
				@Override
				public void keyPressed(KeyEvent e) {
//					System.out.println("keyPressed:"+BaDetailsView.this.getJTextFieldEmail().getText());
				}
		...
		if (jButtonMatchCodeEmail == null) {
			...
			.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						if (pruefeFehler()){
							((BaDetailsController)getController()).holeEMail();
						}
					} catch (SQLException e1) {
						getJComboBoxFehler().addItem(new Fehler(24,FehlerTyp.FEHLER,e1.getMessage()));
						e1.printStackTrace();
