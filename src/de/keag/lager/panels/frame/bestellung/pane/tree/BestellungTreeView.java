...
public class BestellungTreeView extends TreeView implements ITreeBeobachter {
	 ...
	 * 
	 */
	protected BestellungTreeView(BestellungTreeController baTreePaneController) {
		super();
	...
	public void zeichneDich(ModelKnotenBean modelKnotenBean, IModel iModel) {
		...
		if (modelKnotenBean != null) {
			if (modelKnotenBean.getSammelKnotenTypENUM() == ModelKnotenTyp.BESTELLUNG) {
		...
		if (root.getUserObject() instanceof BestellungModelKnotenBean){
			//Anforderung wir im Tree gezeichnet.
			TreePath angeklickterKnoten = null;
			BestellungModelKnotenBean baModelKnotenBean = (BestellungModelKnotenBean)root.getUserObject();
			for (ModelKnotenBean modelKnotenBeanKind : baModelKnotenBean.getKinderList()) {
				if (modelKnotenBeanKind instanceof BestellungPosModelKnotenBean){
	    ...
	    public Component getTreeCellRendererComponent(
			...
			if (node != null && node.getUserObject()!=null) {
				if (node.getUserObject() instanceof ModelKnotenBean) {
					...
					if (modelKnotenBean instanceof BestellungModelKnotenBean) {
						BestellungModelKnotenBean baModelKnotenBean = (BestellungModelKnotenBean)modelKnotenBean;
						String IconNameStatus = BeanDBStatus.JavaToIconName(baModelKnotenBean.getIBean().getBeanDBStatus());
						Icon icon = Run.createCompoundIcon(Konstanten.ICON_BESTELLANFORDERUNG, IconNameStatus,iconError);
						setIcon(icon);
					}
					;
					if (modelKnotenBean instanceof BestellungPosModelKnotenBean) {
						BestellungPosModelKnotenBean baPosModelKnotenBean = (BestellungPosModelKnotenBean)modelKnotenBean;
